...
app = marimo.App(width="full", app_title="(SFT) Discrim Eval Scaling")
...
def _():
    ...
    runs = api.runs(
        "marin-community/marin", filters={"displayName": {"$regex": "tulu3.*sbf.*discrim.*"}, "state": "finished"}
    )
...
def _():
    ...
    def get_model_num_parameters(model_id: str):
        ...
        api = HfApi()
        model_info = api.model_info(model_id.split("_step")[0])
...
def _(
    ...
    def map_run_name_to_model_id(run_name):
        ...
        model_id = model_id.split("@")[0].split("_step")[0].replace("tulu3_", "")
