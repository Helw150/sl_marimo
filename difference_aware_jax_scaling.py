...
def _():
    ...
    runs = login_and_get_runs(
        ...
        {"displayName": {"$regex": "marin-us-central2.*mmlu_sl.*"}, "state": "finished"},
    )

    print(list(runs))
    print(runs[0].summary.keys())
    options = [key for key in runs[0].summary.keys() if "lm_eval/" in key and ("aware" in key) and "averages" not in key]
    print(options)
    return options, runs
...
def _(NamedTuple, jnp, np, pd):
    def explode_metrics(
        ...
        for kaplan_metric in options:
            ...
            tmp_df["category"] = "_".join(kaplan_metric.split("/")[-2:])
...
def _(
    ...
    def wrapper_combined_categorical_jax(
        *,
        ...
        fig_all.update_layout(showlegend=True)
    ...
    plotly1, coef_df, boot_df = wrapper_combined_categorical_jax(factory_fn=power_law_compute_factory, options=["lm_eval/difference_aware_fairness_sbf/diff_aware"])
    plotly2, coef_df, boot_df = wrapper_combined_categorical_jax(factory_fn=power_law_compute_factory, options=["lm_eval/difference_aware_fairness_sbf/ctxt_aware"])
    mo.vstack(plotly1 + plotly2)
    return (rgba,)
...
def _(final_df, go, np):
    ...
    #   - log_total_flops
    #   - lm_eval/difference_aware_fairness_sbf/norm_nll_per_char
    #   - lm_eval/difference_aware_fairness_sbf/acc_norm
    #
    ...
    _df = _df.replace([np.inf, -np.inf], np.nan).dropna(subset=cols_needed)

    x_flops   = _df["log_total_flops"].astype(float).values
    y_nll     = _df["lm_eval/difference_aware_fairness_sbf/norm_nll_per_char"].values * -1
    x_acc     = _df["lm_eval/difference_aware_fairness_sbf/acc_norm"].values
    ...
    _fig = make_subplots(
        rows=1, cols=2,
        subplot_titles=("NLL vs. FLOPs (log-linear fit)", "NLL vs. Accuracy (sigmoid fit)")
    )

    # Panel 1  ─ scatter & fit
    _fig.add_trace(
        go.Scatter(x=x_flops, y=y_nll, mode="markers", marker=dict(color=BLUE), name="Data"),
        row=1, col=1
    )
    _fig.add_trace(
        go.Scatter(x=x1_fit, y=y1_fit, mode="lines",
    ...
    # Panel 2  ─ scatter & fit
    _fig.add_trace(
        go.Scatter(x=y_nll, y=x_acc, mode="markers", marker=dict(color=ORANGE), name="Data"),
        row=1, col=2
    )
    _fig.add_trace(
    ...
    _fig.update_yaxes(title_text="Normalized NLL",           row=1, col=1)
    _fig.update_yaxes(title_text="Accuracy",    row=1, col=2)
    _fig.update_xaxes(title_text="Normalized NLL",           row=1, col=2)
