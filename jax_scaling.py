...
def _(
    ...
    def map_run_name_to_model_id(run_name):
        ...
        model_id = "tomg-group-umd/" + model_id.split("umd-")[-1]
...
def _(NamedTuple, jnp, np, pd):
    def explode_metrics(
        final_df, options, ratios, domain_folder="eval/ICE/", baseline="USA"
    ):
        ...
        for kaplan_metric in options:
            if (
                kaplan_metric.split("/")[-2] not in ratios
                or domain_folder not in kaplan_metric
                or "bpb" not in kaplan_metric
            ...
            tmp_df["relative_bpb"] = (
                tmp_df[f"{domain_folder}{baseline}/bpb"] - tmp_df[kaplan_metric]
            ) / tmp_df[f"{domain_folder}{baseline}/bpb"]
            tmp_df["log_flops"] = np.log10(tmp_df["total_flops"])
            tmp_df["category"] = kaplan_metric.split("/")[-2]
            tmp_df["metric"] = kaplan_metric
            tmp_df["gzip_bpb"] = ratios[kaplan_metric.split("/")[-2]] * 8
...
def _(
    ...
    plotly, coef_df, boot_df = wrapper_combined_categorical_jax()
    mo.vstack(plotly)
    return boot_df, rgba
...
def _(pd):
    ...
    # Fetch the data.
    ed_level = pd.read_csv("https://ourworldindata.org/grapher/world-population-level-education.csv?v=1&csvType=full&useColumnShortNames=true", storage_options = {'User-Agent': 'Our World In Data data fetch/1.0'})

    # Fetch the metadata
    ed_metadata = requests.get("https://ourworldindata.org/grapher/world-population-level-education.metadata.json?v=1&csvType=full&useColumnShortNames=true").json()
    return (ed_level,)


@app.cell
def _(boot_df, ed_level, mo, np, pd):
    from scipy import stats
    import plotly.express as px

    # Fixed version with common issues addressed
    join_ed = ed_level[ed_level["Year"] == 1990].copy()
    pop_csv = pd.read_csv("pop.csv")
    gdp = pd.read_csv("./int_capita.csv")
    gdp["Country Name"] = gdp["Country Name"].str.replace("SAR, China", "").str.strip()
    gdp = gdp[["Country Name", "2000"]].rename(columns={"2000": "% Internet Users"})
    join_ed = join_ed.merge(gdp, left_on="Entity", right_on="Country Name", how='left')
    pop_csv["Population"] = pop_csv["Population"].str.replace(",", "").astype(int)
    pop_csv["Country / Dependency"] = pop_csv["Country / Dependency"].str.replace("(China)", "").str.replace("(UK)", "").str.strip()
    join_ed = pop_csv.merge(join_ed, left_on='Country / Dependency', right_on='Entity', how='left')
    join_ed["join"] = "b[" + join_ed["Country / Dependency"].str.replace(" ", "_") + "]"
    join_ed["Internet Users"] = (join_ed["% Internet Users"] * join_ed["Population"])
    join_ed["Log Internet Users"] = np.log10(join_ed["Internet Users"])
    #join_ed["Internet Users"] = np.log((join_ed["Internet Users"] / join_ed.loc[join_ed["Entity"] == "United States", "Internet Users"].iloc[0]))

    # Set the join column as index for proper joining
    join_ed = join_ed.set_index("join")
    join_ed = join_ed.dropna(subset=["Internet Users"])
    #join_ed["Internet Users"] = stats.zscore(join_ed["Internet Users"])


    # Get summary statistics
    summ = boot_df.describe(percentiles=[0.025, 0.5, 0.975]).T

    # Filter and sort slopes
    slopes = (summ[summ.index.str.contains("b\[|gzip", na=False)]
              .sort_values(by="50%") * -1)

    # Join with proper index alignment
    result = slopes.join(join_ed, how='left')[["50%", "Internet Users", "Log Internet Users"]].dropna().rename(columns={"50%": "Scaling Law Slope"})

    # Display the result
    _main_df = mo.ui.dataframe(result, page_size=15)
    _corr, _p_val = stats.pearsonr(result.dropna()["Scaling Law Slope"], result.dropna()["Log Internet Users"])
    _corr_str = mo.md(f"### Pearson Correlation between slope of scaling law and the Log(Internet Users): {_corr} (P={_p_val})")
    _scatter_fig = px.scatter(
        ...
        width=500,
        height=500,
        hover_data=result.reset_index().columns,
        title="Scaling Law Slope v.s. Log(Internet Users)",
        labels={"Scaling Law Slope": "Scaling Law Slope", "Internet Users": "Internet Users"},
        template="plotly_white",
        trendline="ols",
        trendline_options=dict(log_x=True),
    )

    # Update trace and trendline to use Stanford grey
    STANFORD_GREY = "#53565A"
    _scatter_fig.update_traces(marker=dict(color=STANFORD_GREY))
    _scatter_fig.update_traces(
        selector=dict(mode='lines'),
        line=dict(color=STANFORD_GREY, dash='dash')
    )

    #_scatter_fig.update_xaxes(autorange="reversed")




    mo.hstack([mo.vstack([_corr_str, _scatter_fig]), _main_df])
    return
